...
"R:" 
cor(pld_subset$CreditScoreRangeLower,pld_subset$BorrowerAPR, use = "complete.obs")
```
...
paste("R:", cor(pld_subset.verificado$StatedMonthlyIncome,pld_subset.verificado$BorrowerAPR, use = "complete.obs"))
```
...
Nota-se uma grande mudança nos valores da taxa de juros de a pessoa tem ou não casa. tivemos 0.02 de diferença na taxa de juros média. com um ICC de apenas 0.032
...
"R:" 
cor(pld_subset.verificado$DebtToIncomeRatio,pld_subset.verificado$BorrowerAPR, use = "complete.obs")
```
...
"R:" 
cor(pld_subset$OpenRevolvingAccounts,pld_subset$BorrowerAPR, use = "complete.obs")
```
...
paste("ICC Calculation: ", ICCbare(data=pld_subset.verificado, x = pago, y = StatedMonthlyIncome))
```
...
#print(paste("ICC Calculation: ", ICCbare(data=pld_subset.verificado, x = pago, y = CreditScoreRangeLower)))
```
...
#print(paste("ICC Calculation: ", ICCbare(data=pld_subset.verificado, x = pago, y = DebtToIncomeRatio)))
```
...
paste("R:", cor(pld_subset$DebtToIncomeRatio,pld_subset$CreditScoreRangeLower, use = "complete.obs"))
```
...
ggplot(pld_subset.verificado,aes(x=StatedMonthlyIncome,y=CreditScoreRangeLower))+ geom_point(alpha=0.05)+xlim(quantile(pld_subset$StatedMonthlyIncome,probs=0.01,na.rm=TRUE),quantile(pld_subset$StatedMonthlyIncome,probs=0.99,na.rm=TRUE))+ ylim(quantile(pld_subset$CreditScoreRangeLower,probs=0.01,na.rm=TRUE),quantile(pld_subset$CreditScoreRangeLower,probs=0.99,na.rm=TRUE))+
  geom_smooth()
paste("R:", cor(pld_subset.verificado$StatedMonthlyIncome,pld_subset.verificado$CreditScoreRangeLower, use = "complete.obs"))
```
...
paste("R:", cor(pld_subset.verificado$TradesNeverDelinquent..percentage.,pld_subset.verificado$CreditScoreRangeLower, use = "complete.obs"))
```
...
paste("R:", cor(pld_subset.verificado$TradesNeverDelinquent..percentage.,pld_subset.verificado$BorrowerAPR, use = "complete.obs"))
```
...
pld_subset%>% group_by(pago,ListingCategory..numeric.) %>%
  tally %>%
  group_by(ListingCategory..numeric.) %>%
    mutate(pct =round(100*n/sum(n),0)) %>% arrange(pago,desc(pct)) %>% 
  ggplot(aes(x=ListingCategory..numeric., y = pct,color = pago,label=pct))+geom_bar(stat="identity", width = 0.85) + geom_text(position="stack",
     aes(ymax=2),vjust=1) +ylab("porcentagem de bons ou maus pagadores na categoria (%)")+xlab("Motivos para o empréstimo")+labs(title = "Porcentagem de bons pagadores para cada motivo de empréstimo")
```
...
ggplot(pld_subset, aes(x=CreditScoreRangeLower,y=BorrowerAPR*100, color=pago, group= pago )) + geom_point(aes(shape=pago), alpha=0.05)+
  xlim(quantile(pld_subset$CreditScoreRangeLower,probs=0.01,na.rm=TRUE),quantile(pld_subset$CreditScoreRangeLower,probs=0.99,na.rm=TRUE))+ geom_smooth()+ ylab("Taxa de Juros Anual (%)") + xlab("Credit Score") + labs(title = "Taxa de Juros Anual em Relação ao Credit Score Agrupado \n Entre Maus e Bons Pagadores") +
...
ggplot(pld_subset.verificado, aes(x=StatedMonthlyIncome,y = (BorrowerRate*100), group=interaction(IsBorrowerHomeowner, empregado ), color = interaction(IsBorrowerHomeowner, empregado))) + geom_point( alpha=0.01)+ 
  xlim(quantile(pld_subset.verificado$StatedMonthlyIncome,probs=0.01,na.rm=TRUE),quantile(pld_subset.verificado$StatedMonthlyIncome,probs=0.99,na.rm=TRUE))+geom_smooth(aes(color = interaction(IsBorrowerHomeowner, empregado)))+ labs(color= "Casa Própria, Empregado")+ ylab("Taxa de Juros (%)") + xlab("Renda Mensal Verificada ($)")+labs(title="Taxa de Juros em Relação a Casa Própria, \n Status de Emprego e Renda Mensal")+
  theme(plot.title = element_text(hjust = 0.5))
```
